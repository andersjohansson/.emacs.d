...
(use-package ox-reveal
  :defer t
  :load-path "~/kodat/elisp/org-reveal")
...
(when aj/ajeb
  (use-package mu4e
	...
	:defer t
	:init (setq mu4e-get-mail-command "mailsync" ; bash script running 3 offlineimap
...
(use-package dired-hide-dotfiles
  ...
  (with-eval-after-load 'dired
    (bind-key "å" #'dired-hide-dotfiles-mode dired-mode-map)))
#+END_SRC
**** Dired subtree
#+BEGIN_SRC emacs-lisp
...
But also my yet ([2018-03-01]) unmerged fixes:
...
(with-eval-after-load "tex-mode"
  (setq tex--prettify-symbols-alist (delete '("\\par" . ? ) tex--prettify-symbols-alist)))
#+END_SRC
...
(use-package helm
  ...
  (setq helm-display-function #'aj/helm-display-child-frame
...
(defun aj/add-favourite-file ()
  ...
  (when (buffer-file-name)
    (add-to-list 'aj/favourite-files (buffer-file-name) nil #'string=)))
...
(use-package company-box
  :load-path "~/kodat/elisp/company-box" ;; fork from https://github.com/a13/company-box
  :ensure t
  :delight ""
...
*** Text-mode uses visual-line-mode
#+begin_src emacs-lisp
...
Editing abbrevs with ~edit-abbrevs~ and saving them in the abbrev-file doesn’t seem to preserve properties. Therefore properties are added below.
#+BEGIN_SRC emacs-lisp
(use-package abbrev
  :delight
  :init
...
**** Avoid unneccesary messages :ARCHIVE:
#+BEGIN_SRC emacs-lisp
...
My extra libraries can be found at:  https://gitlab.com/andersjohansson/define-word-thesaurus (doesn’t work with updated thesaurus.com) and  https://gitlab.com/andersjohansson/definew-word-tyda.

I like thesaurus.com because it separates different meanings of a word, and because it sorts the words into more and less common.
Thesaurus.com has changed the frontend to random class names and ids however, and is much harder to parse now than when i wrote ~define-word-thesaurus~.
https://github.com/Manwholikespie/thesaurus (python) seems to be doing a good job of it though. If it was easy and my system didn’t crash when attempting to call python from elisp (with https://github.com/jvkersch/emacs-python) it would be a very good alternative. Parsing the output from a python shell seems to be an alternative for using that.
...
(use-package synosaurus
  ...
  :init
  ;; this means completing-read, which gives me helm completion
...
(defhydra hydra-word-lookup
  ...
  ("S" synosaurus-choose-and-replace "Synosaurus at point")
...
*** all the icons
**** Base package
When this fix https://github.com/ubolonton/all-the-icons.el/tree/font-lock-fix is implemented I could use it from melpa.
#+BEGIN_SRC emacs-lisp
...
Also runs hook aj/face-definitions-hook."
  (unless (eq theme 'user) ;enable theme calls itself with user theme.
    (with-eval-after-load 'org-faces
      ...
      ;;(let ((shadow-color (face-attribute 'highlight :background)))
      (set-face-attribute 'org-block-begin-line nil :inherit '(shadow) :height 0.9)
      (set-face-attribute 'org-block-end-line nil :inherit '(shadow)  :height 0.9)
      ;;)
