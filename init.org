...
*** Tooltip-mode
I want to use the echo area (hence ~(tooltip-mode -1)~), but don’t want help echo messages to be truncated, so I need to patch ~tooltip-show-help-non-mode~.
#+begin_src emacs-lisp
(tooltip-mode -1)

(with-eval-after-load 'tooltip
  (el-patch-defun tooltip-show-help-non-mode (help)
    ...
    (when (and (not (window-minibuffer-p)) ;Don't overwrite minibuffer contents.
      ...
      (cond
       ((stringp help)
        ...
        (let ((message-truncate-lines (el-patch-swap t nil))
              (message-log-max nil))
          (message "%s" help)))
...
Also runs hook aj/face-definitions-hook."
  (unless (eq theme 'user) ;enable theme calls itself with user theme.
    ...
    (with-eval-after-load 'org-faces
      ...
      (set-face-attribute 'org-tag nil :inherit 'shadow)
...
(use-package gruvbox
  :straight (gruvbox-theme
             :depth nil
             :fork (:host github :repo "andersjohansson/emacs-theme-gruvbox" :branch "ajfixes")))
#+END_SRC
