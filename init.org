...
(straight-use-package '(org :host github
                            ...
                            :files (:defaults
                                    "lisp/*.el"
                                    ("etc/styles/" "etc/styles/*")))) ;;TODO update with csl-stuff
...
(use-package helm
  ...
  (advice-add 'helm-highlight-buffers :override #'aj/helm-highlight-buffers)
  (defun aj/helm-highlight-buffers (buffers _source)
...
(use-package orgqda
  :straight (:depth full :host gitlab :repo "andersjohansson/orgqda" :files ("orgqda.el"))
  :defer t
  :after org
  ...
  (with-eval-after-load 'orgqda-helm-tags
    (define-key orgqda-helm-tags-mode-map [remap org-set-tags-command]
...
(use-package org
  ...
  (setq org-directory "~/org"
        org-ellipsis " ⤵"
        org-export-backends '(odt ascii html icalendar latex md)
...
(use-package password-store
  ...
  (defun aj/password-store-generate-and-copy (&optional replace)
    ...
    (let ((entry (aj/password-store-get-new-entry-with-completion replace)))
      (password-store--run-generate entry nil replace)
...
*** Selected: a custom keymap when the region is active
...
(use-package ediff
  ...
  ;;ediff expects diff to return results in C-locale, I’ve made a
  ;;wrapper script to solve this (script is: LANG=C diff "$1" "$2")
  (ediff-diff-program "aj-diff-no-locale")
  :config
...
(use-package pdf-zotxt-quote
  :straight (:depth full :host gitlab :repo "andersjohansson/pdf-zotxt-quote")
...
*** Cycle font size
Sometimes I feel the need for a generally bigger font in all emacs frames. Especially when switching between my 13 inch 1920*1080-screen and my 24 inch 1920*1080-screen (systemwide screenwise fractional scaling is still not an option on linux). On the small screen I sometimes toogle text-scaling in gnome, and if emacs is started with that setting it is kept so the font is too large when switching to the large screen. Then I can also cycle to a smaller size.
#+begin_src emacs-lisp
(defvar aj/orig-font-size nil)
(defvar aj/orig-gnome-font-size nil)

(add-hook 'window-setup-hook
          (lambda () (setq aj/orig-font-size
                      (face-attribute 'default :height)
                      aj/orig-gnome-font-size
                      (string-to-number
                       (shell-command-to-string
                        "gsettings get org.gnome.desktop.interface text-scaling-factor")))))

(defun aj/set-font-size-following-gnome (scaling-factor)
  (let ((height (round (* aj/orig-font-size (/ scaling-factor aj/orig-gnome-font-size)))))
