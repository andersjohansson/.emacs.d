...
***** Vertico for the interface and orderless as completion style
#+begin_src emacs-lisp
(use-package vertico
  :straight (vertico :includes (vertico-directory vertico-repeat)
                     :files (:defaults
...
(use-package consult
  ...
  (consult-ripgrep-args
   "rga --no-ignore --null --line-buffered --color=never --max-columns=1000 --path-separator / --smart-case --no-heading --line-number .")
  ...
  ;; CRM
  ;; https://github.com/bdarcus/citar/issues/492#issuecomment-997024450
          ...
          (max 0 ; if none above, choose the first below
               (1- (or (seq-position
                        ...
                        (lambda (cand point-pos) ; counts on candidate list being sorted
                          (> (car (get-text-property ; get text location marker
                                   0
                                   (cl-case current-minibuffer-command
                                     (consult-org-heading 'consult-org--heading)
                                     (consult-outline 'consult-location))
                                   cand))
                             point-pos)))
...
(defun aj/open-zoterofile-new-frame ()
  (let ((selectrum-display-action '(selectrum-display-full-frame)))
...
(use-package org
  ...
  (setq org-directory "~/org"
        org-ellipsis " ⤵"
        org-export-backends '(org odt ascii html icalendar latex md md-pandoc-zotero)
...
(embark-define-keymap embark-word-definitions-map
...
(fset 'embark-word-definitions-map embark-word-definitions-map)
(define-key embark-symbol-map "w" 'embark-word-definitions-map)

;; (bind-key "C-x l" #'aj/embark-word-definitions)


;; (defun aj/embark-word-definitions ()
;;   (interactive)
;;   (let* ((ind '(embark-mixed-indicator embark-highlight-indicator))
;;          (targets (embark--targets))
;;          (tf (seq-filter (lambda (x)
;;                            (member (plist-get x :type) '(symbol region)))
;;                          targets))
;;          (become (unwind-protect
;;                      (embark--prompt
;;                       (mapcar #'funcall ind)
;;                       embark-word-definitions-map
;;                       (or targets
;;                           `((:type embark-become :target ,(read-string "Word: ")))))
;;                    (mapc #'funcall ind))))
...
Also runs hook aj/face-definitions-hook."
  (unless (member theme '(use-package user)) ;enable theme calls itself with user theme.
    ...
    ;; (when (eq 'pgtk (framep-on-display))
    ;;   (set-face-attribute 'internal-border nil :background "#dddddd"))
    ...
    (with-eval-after-load 'org-faces
      ...
      (set-face-attribute 'org-block-end-line nil :underline (modus-themes-color 'bg-active) :overline nil :height 0.9)
      ...
      (when (eq theme 'modus-operandi)
        ...
        (set-face-attribute 'org-todo nil :height 0.8 :inherit 'fixed-pitch)

        (defface aj/org-todo `((t :inherit 'org-todo :foreground ,(modus-themes-color 'red))) "Face for todo org-state" :group 'org-faces)
        (defface aj/org-waiting `((t :inherit 'aj/org-todo :foreground ,(modus-themes-color 'yellow) :inherit fixed-pitch)) "Face for waiting org-state" :group 'org-faces)
        (defface aj/org-started `((t :inherit 'aj/org-todo :foreground ,(modus-themes-color 'blue))) "Face for started org-state" :group 'org-faces)
        (defface aj/org-done `((t :inherit 'aj/org-todo :foreground ,(modus-themes-color 'green) :inherit fixed-pitch)) "Face for done org-state" :group 'org-faces)
