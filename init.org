...
(use-package consult
  ...
  (consult-ripgrep-args
   "rg --no-ignore --null --line-buffered --color=never --max-columns=1000 --path-separator / --smart-case --no-heading --line-number .")

  (consult-preview-key (kbd "C-å"))
  :init
...
(use-package orgqda
  ...
  (define-key orgqda-mode-map [remap org-set-tags-command] #'aj/orgqda-tag-or-code)
  ...
  (unless (member theme '(use-package user)) ;enable theme calls itself with user theme.
    ...
    (with-eval-after-load 'org-faces
      ...
      (when (eq theme 'modus-operandi)
        ...
        ;; (defface aj/org-todo `((t :foreground ,(modus-themes-color 'red))) "Face for todo org-state" :group 'org-faces)
        ;; (defface aj/org-waiting `((t :foreground ,(modus-themes-color 'yellow) :inherit fixed-pitch)) "Face for waiting org-state" :group 'org-faces)
        ;; (defface aj/org-started `((t :foreground ,(modus-themes-color 'blue))) "Face for started org-state" :group 'org-faces)
        ;; (defface aj/org-done `((t :foreground ,(modus-themes-color 'green) :inherit fixed-pitch)) "Face for done org-state" :group 'org-faces)
