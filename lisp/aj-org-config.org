...
(use-package citar
  ...
  (citar-insert-reference-function #'aj/citar-get-csl-references)
  :config
  (citar-filenotify-setup nil)
  (define-key citar-citation-map (kbd "z") (cons "open in zotero" #'aj/org-cite-zotxt-follow))
  (define-key citar-citation-map (kbd "F") (cons "open externally" #'aj/citar-open-files-externally))
  (defun aj/citar-open-files-externally (keys)
    "Open citation KEYS in external program."
    (let ((citar-file-open-function #'citar-file-open-external))
      (citar-open-library-files keys)))
