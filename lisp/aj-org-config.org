...
(defun aj/org-insert-macro ()
  ...
  (insert
   (format
    ...
    (helm-comp-read
     "Macro: "
     (cl-loop for (m . e) in org-macro-templates
              collect
              (cons
               (concat m (propertize (concat " (" e ")") 'face 'shadow))
               m))
     :multiline t))))
#+end_src
...
(use-package bibtex-actions
  ...
  (bibtex-actions-open-library-file-external nil)
  (bibtex-actions-file-open-function #'find-file-other-window)
  :config
  (bibtex-actions-filenotify-setup nil))
...
(use-package oc-bibtex-actions
  ...
  (define-key oc-bibtex-actions-map (kbd "z") '("open in zotero" . aj/org-cite-zotxt-follow))
  (define-key oc-bibtex-actions-map (kbd "F") '("open externally" . aj/bibtex-actions-open-files-externally))

  (defun aj/bibtex-actions-open-files-externally (keys)
    "Open citation KEYS in external program."
    (let ((bibtex-actions-open-library-file-external t))
      (bibtex-actions-open-library-files keys))))
...
*** Exporting
See also [[file:~/.emacs.d/init.org::*ox-md-pandoc-zotero]] for the zotero-conversion export
...
**** Split bibliography across reveal.js slides
We install an output filter for doing this.
#+begin_src emacs-lisp
...
I use variable-width fonts, so trying to align tags to the right using spaces is useless. But in the agenda we want them right aligned and setting it in the hook means we can adapt to the current window.
