...
#+end_src
** [[https://github.com/nullman/emacs-org-visibility][org-visibility]]
...
(use-package org-annotate
  ...
  (set-face-attribute 'org-annotate-face nil :underline nil :inherit 'font-lock-comment-face)
...
(use-package citar-org
  ...
  :custom (citar-org-activation-functions '(org-cite-csl-activate citar-org-activate-keymap))
  :config
  ...
  (format "<blockquote%s>\n%s</blockquote>"
	      (let* ((reference (org-html--reference quote-block info t))
                 ...
                 (attributes (if caption
                                 (plist-put attributes :cite (org-trim (org-export-data caption info)))
                               attributes))
