...
(use-package org-modern
  ...
  :custom
  (org-modern-block nil) ;; doesn’t work with org-indent
...
*** Do org-indent indentation with display spaces
...
#+begin_src emacs-lisp
;; do org-indent late so that it happens after we have variable-pitch
;; etc. (org-startup-indented is nil)
...
(defun aj/embark-org-clone-indirect-heading (heading)
  (when-let ((m (car-safe (get-text-property 0 'consult-org--heading heading))))
    (let ((consult-after-jump-hook nil))
      (save-excursion
        (save-restriction
          (consult--jump-nomark m)
...
Possibly passss INITIAL-INPUT"
  (let* ((ref (citar-select-ref :initial-input initial-input)))
    (setq aj/org-last-returned-citation-key (car ref))
...
(use-package citar
  ...
  (citar-open-note-function #'aj/citar-open-roam-note)
  :config
  ...
  (defun aj/citar-open-file-externally (key-entry)
    ...
    (let ((citar-file-open-function #'citar-file-open-external))
      (citar-open-library-file key-entry)))
  ...
  (add-hook 'citar-force-refresh-hook #'aj/citar-fetch-zotero-json)

  (defun aj/citar-open-roam-note (key _entry)
    (org-roam-ref-find key))
...
#+end_src
*** Add extra group for current refs to citar
#+begin_src emacs-lisp
...
However, we can also use the display text property to align tags to the right of the window. This is already done in the agenda when setting ~org-agenda-tags-column~ is ~auto~, the default.
