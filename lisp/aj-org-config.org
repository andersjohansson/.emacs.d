...
*** Mode with some extra bindings for reading, reviewing and linking literature
#+begin_src emacs-lisp
(define-minor-mode aj/literature-mode
  "Binds some useful functions for literature reading."
  :keymap
  (define-keymap
    "C-ä f" #'aj/find-citar-file
    "C-ä l" #'aj/zotero-add-link-to-headline
    "C-ä r" #'aj/zotero-add-link-mark-read
    "C-ä c" #'aj/org-roam-create-notes))

(defun aj/find-citar-file ()
  "Find citar file of entry under point, or reference for current headline."
  (interactive)
  (when-let ((k (or
                 (citar-org--key-at-point)
                 (citar-org--prop-key-at-point)
                 (when-let ((node (org-roam-node-at-point))
                            (key (car (org-roam-node-refs node))))
                   (cons key nil)))))
    (citar-open-files k)))
#+end_src

** org-similarity :ARCHIVE:
Doesn’t seem to work to well with node-based workflow for org-roam.
#+begin_src emacs-lisp
(use-package org-similarity
  :straight ( :host github
              :repo "brunoarine/org-similarity"
              :branch "develop"
              :files ("*.el" "requirements.txt" "orgsimilarity" "tests" "eval.py" "Makefile" "pyproject.toml" "setup.py"))
  :custom
  (org-similarity-directory org-roam-directory)
  (org-similarity-language "Swedish")
...
(use-package org-modern
  ...
  ;; I define a static definition for org-modern-label in aj-theme.el,
  ;; and don’t use the table style, so this function is unneccesary.
  (advice-add 'org-modern--update-label-face :override #'ignore)
  (advice-add 'org-modern--pre-redisplay :override #'ignore)
  :custom
  ;; otherwise windows with different font sizes will mess it up for each other:
  (org-modern-label-border 3)
  ;; not needed when using ‘org-indent-mode’ which implies ‘org-hide-leading-stars’:
...
*** [[https://github.com/jdtsmith/org-modern-indent][org-modern-indent]] :ARCHIVE:
#+begin_src emacs-lisp
	       ...
	       (wrap
	        (org-add-props
                (el-patch-swap
                  ...
                  (if heading
                      (aref org-indent--text-line-prefixes level)
...
Keywords etc. are completed with pcomplete when typing #+ and TAB/M-TAB

Structure templates works well to insert with ~org-insert-structure-template~ (~C-c C-,~).

Disregard old solution below that used org-tempo and hydra.

#+begin_src emacs-lisp
(advice-add 'org-pcomplete-case-double :override #'aj/org-pcomplete-case-down)
(defun aj/org-pcomplete-case-down (list)
  "Return list with downcase version of all strings in LIST."
...
**** Old solution with org-tempo and hydra. :ARCHIVE:
#+begin_src emacs-lisp
...
;;   (add-to-list 'aj/org-prettify-symbols kw))
#+end_src
**** Let org-shift-left/right switch checkbox state instead of bullet type for lists :ARCHIVE:
#+begin_src emacs-lisp
(defun aj/org-shift-checkbox (arg)
  (interactive "P")
  (org-toggle-checkbox))

(defun aj/org-shift-left-right-list-checkbox (fun &optional arg)
  (cl-letf (((symbol-function 'org-cycle-list-bullet)
             #'aj/org-shift-checkbox))
    (funcall fun arg)))

(advice-add 'org-shiftleft :around #'aj/org-shift-left-right-list-checkbox)
(advice-add 'org-shiftright :around #'aj/org-shift-left-right-list-checkbox)
...
(defun aj/org-add-todo-and-cookie (maxlevel state)
  "Add TODO STATE if not in a TODO state and a [/]-cookie if there are children."
...
**** COMMENT External calendars :ARCHIVE:
Had trouble with these, disabling
...
(defun aj/org-calendar-maybe-sync-all (&optional force)
  ...
  (deferred:$
   (deferred:parallel
    ...
    ;; doesn’t work anymore since transition to O365
...
#+begin_src emacs-lisp
(defvar-keymap aj/org-heading-map
  :doc "Keymap for org mode headings"
...
** org-protocol for linking into org files from Zotero + functions for adding/removing tags
...
(defun aj/zotero-zotaj-request (endpoint params &optional id)
  "Do a zotero zotaj request for key at point, if found.
ENDPOINT specifies the endpoint and PARAMS additional
parameters. ID is the key."
  (if-let ((id (or id (aj/citar-org-key-at-point-or-headline))))
      (request
        (format "http://127.0.0.1:23119/zotaj/%s" endpoint)
        :params
        `(("betterbibtexkey" . ,(substring-no-properties (car id)))
          ,@params)
        :success (cl-function
    ...
    (user-error "Couldn’t find key at point!")))

;; (defun aj/zotero-add-tag-to-current-item (tag &optional id)
;;   (interactive "sTag: ")
;;   (aj/zotero-zotaj-request "addtag" `(("tag" . , tag)) id))

;; (defun aj/zotero-add-remove-tag-from-current-item (tag &optional id)
;;   (interactive "sTag: ")
;;   (aj/zotero-zotaj-request "removetag" `(("tag" . , tag)) id))

(defun aj/zotero-add-link-to-headline (&optional id)
  (interactive)
  (if-let ((link (aj/get-org-protocol-link))
           (fn (buffer-file-name (org-base-buffer (current-buffer)))))
      (aj/zotero-zotaj-request
       "inserturl"
       `(("url" . ,link)
...
(defun aj/zotero-zotaj-request-deferred (endpoint params &optional id)
  "Do a zotero zotaj request for key at point, if found.
ENDPOINT specifies the endpoint and PARAMS additional
parameters. ID is the key."
  (if-let ((id (or id (aj/citar-org-key-at-point-or-headline))))
      (request-deferred
       ...
       `(("betterbibtexkey" . ,(substring-no-properties (car id)))
         ,@params)
       :success (cl-function
                 (lambda (&key response _data &allow-other-keys)
                   (message "Success with %s!" (request-response-url response))))
       :error (cl-function
               (lambda (&key response &allow-other-keys)
                 (message "Failed on %s" (request-response-url response)))))
    (user-error "Couldn’t find key at point!")))

(defun aj/zotero-add-link-mark-read ()
  "Add link to headline from zotero and tag item as read."
  (interactive)
  (if-let ((id (aj/citar-org-key-at-point-or-headline)))
      (deferred:$
       (aj/zotero-zotaj-request-deferred "addremovetags"
                                         `(("addtags" . "✔️")
                                           ("removetags" . "➡️;〰️️️"))
                                         id)
       (deferred:wait 1000)
       (if-let ((link (aj/get-org-protocol-link))
                (fn (buffer-file-name (org-base-buffer (current-buffer)))))
           (aj/zotero-zotaj-request-deferred
            "inserturl"
            `(("url" . ,link)
              ("title" . ,(format "org-link to %s" fn)))
            id)
...
** Drawers for odt-comments :ARCHIVE:
Inspired by https://list.orgmode.org/878rqj4gmm.fsf@ucl.ac.uk/
#+begin_src emacs-lisp
;; (setq org-odt-format-drawer-function #'aj/ox-odt-comment-drawer)
;; (defun aj/ox-odt-comment-drawer (name contents)
;;   (if (string= name "comment")
...
(use-package org-annotate
  ...
  (set-face-attribute 'org-annotate-text-face nil :underline nil :weight 'unspecified :inherit 'modus-themes-subtle-yellow)
...
(use-package citar
  ...
  ;; uppercase field names, in my zotero-export. Only use doi and url.
  ...
  (define-key citar-citation-map (kbd "z") (cons "open in zotero" #'citar-open-entry-in-zotero))
  ...
  (defun aj/citar-copy-references-new-frame ()
    (interactive)
    (aj/run-cmd-in-small-frame 100 20 "References" #'citar-copy-reference :interactively t :force-cleanup t))
  )
...
(use-package citar-org
  ...
  (defun aj/citar-org-key-at-point-or-headline ()
    "Return key at point for org-cite citation-reference, property, or roam-ref."
...
(use-package citar-org-roam
  ...
  (defvar aj/org-roam-ref-note-template
    "* ${title} :#läsning:
:PROPERTIES:
:ID: ${id}
:ROAM_REFS: ${refs}
...
%?")
  ...
  (defun aj/org-roam-note-node (citekey)
    "Create template node"
    (org-roam-node-create
     :title (citar-format--entry citar-org-roam-note-title-template citekey)
     :id (org-id-new)
     :refs (concat "@" citekey)))

  (defun aj/org-roam-create-notes (citekeys)
    "Create and insert note templates at point, for CITEKEYS."
    (interactive (list (or (citar-citation-at-point)
                           (citar-select-refs))))
    (cl-loop for key in citekeys
             concat
             (let ((org-roam-capture--node (aj/org-roam-note-node key)))
               (concat (org-roam-capture--fill-template aj/org-roam-ref-note-template)
                       "\n")))
    (org-paste-subtree
     (1+ (org-current-level)) ;; hack but works.
     (cl-loop for key in citekeys
              concat
              (let ((org-roam-capture--node (aj/org-roam-note-node key)))
                (concat (org-roam-capture--fill-template aj/org-roam-ref-note-template)
  ...
  (defun aj/citar-org-roam--create-capture-note (citekey entry)
    ...
    (org-roam-capture-
     :templates
     `(("r" "reference" entry ,aj/org-roam-ref-note-template
        :target
        (file+datetree "~/jobb/anteckningar/anteckningar-%<%Y>.org" month)
        :empty-lines-before 1))
     :node (aj/org-roam-note-node citekey)))
...
;; ("l" "Läsanteckningar" entry "* %(aj/org-return-reference-author-year-title \"${title}\") :#läsning:
;; :PROPERTIES:
;; :ID: %(org-id-new)
...
;; %u
;; %i
;; %?" :empty-lines-before 1 :target
;; (file+datetree "~/jobb/anteckningar/anteckningar-%<%Y>.org" month))
#+end_src
*** Merge consecutive citations
A simple package for this, available at [[https://git.sr.ht/~andersjohansson/org-cite-merge][sourcehut]].
#+begin_src emacs-lisp
(use-package org-cite-merge
  :straight (org-cite-merge :host sourcehut
                            :repo "andersjohansson/org-cite-merge"))
#+end_src
...
;;   (let ((bibtex-completion-pdf-open-function #'embark-open-externally))
;;     (bibtex-completion-open-pdf
;;      (aj/org-cite-get-citekeys datum))))
...
(setq org-cite-export-processors
      '(;; (latex biblatex)
        (pandoc pandoc)
        ;; because I have fancy footnote-display for reveal:
        ;; experimental fork for this:
        ;; https://gitlab.com/andersjohansson/org-re-reveal/-/tree/footnotes
        (re-reveal . (csl "chicago-fullnote-bibliography.csl" "chicago-fullnote-bibliography.csl"))
...
(defun aj/org2-cleanup-html ()
  "Clean up html in current buffer"
  (interactive)
  (goto-char (point-min))
  (flush-lines "</?div.+class=\"outline")
  (cl-loop for (r . rr) in '((" class=\"org-ul\"" . "")
                             (" id=\"[^\"]+\"" . "")
                             ("\n\n" . "\n")
                             ("<p>[[:space:]]</p>" . ""))
           do
...
(with-eval-after-load 'org-re-reveal
  (define-minor-mode aj/org-reveal-mode
    ...
    :keymap `((,(kbd "C-c C-'") . aj/hydra-org-reveal-templates/body)))
          ...
          (concat "#+attr_html: :class "
                  (mapconcat
                   ...
                   (completing-read-multiple
                    "Class: " (aj/org-reveal-get-css-classes))
                   " ")))
...
(defvar aj/org-reveal-extra-classes
  '(("r-stack"
     "r-stretch"
     "r-fit-text")))

(defun aj/org-reveal-get-css-classes ()
  (let ((files '("/home/aj/jobb/presentationer/assets/ajcommon.css"
                 "/home/aj/jobb/presentationer/assets/ajwhite.css"
                 "/home/aj/jobb/presentationer/assets/aj-contrast-backgrounds.css"))
        (filterfiles '("~/lib/reveal.js/css/layout.scss"
                       "~/lib/reveal.js/css/reveal.scss"
                       "~/lib/reveal.js/css/theme/source/white.scss")))
    (append
     aj/org-reveal-extra-classes
     aj/reveal-wrap-classes
     (seq-difference
      (seq-uniq (mapcan #'aj/org-reveal-get-css-classes-in-file files) #'equal)
      (seq-uniq (mapcan #'aj/org-reveal-get-css-classes-in-file filterfiles) #'equal)
      #'equal))))

(defun aj/org-reveal-get-css-classes-in-file (file)
  (let (clist)
    (with-temp-buffer
    ...
    (org-element-map data 'headline
      (lambda (h)
        (when (member "notitle" (org-element-property :tags h))
          ...
          (org-element-put-property
           h :reveal_data_state
           (concat (org-element-property :reveal_data_state h) " notitle"))))
      info nil))
  data)
...
**** Minify css and js
#+begin_src emacs-lisp
(defun aj/org-re-reveal--read-file-as-string (filename &optional raise-error)
        ...
        (let ((format-alist nil)
          ...
          (cond
           ((string-match "\\.css$" filename)
            (call-process-shell-command (concat "python -mrcssmin <" filename) nil t))
           ((string-match "\\.js$" filename)
            (call-process-shell-command (concat "python -mrjsmin <" filename) nil t))
...
(advice-add 'org-re-reveal--read-file-as-string :override #'aj/org-re-reveal--read-file-as-string)
...
#+begin_src emacs-lisp
(setq
 ;; usually don’t use math, and stupid to always load mathjax
...
**** CV
#+begin_src emacs-lisp
(defvar-local aj/org-exporting-cv nil)

(advice-add 'org-html-item :around #'aj/org-html-item-cv)

;; (defun aj/org-html-item-cv (fun item contents info)
;;   (if (and aj/org-exporting-cv
;;            (eq 'descriptive
...
;;              (tag (or tag "")))
;;         (concat
;;          (concat (format "<div><dt>%s</dt>" tag)
;; 		         "<dd>"
;;                  (cl-loop for f being the elements of
;;                           (split-string contents ";" nil  "[ \n]+")
;;                           using (index i)
;;                           concat
...
;;                                   f)))
;;          "</dd></div>\n"))
;;     (funcall fun item contents info)))
...
(advice-add 'org-html-plain-list :around #'aj/org-html-plain-list-cv)

(defun aj/org-html-plain-list-cv (fun plain-list contents info)
  (if (and aj/org-exporting-cv
           (eq 'descriptive
               (org-element-property :type plain-list)))
      (format "<table class=\"cv\">%s</table>" contents)
    (funcall fun plain-list contents info)))

(advice-add 'org-html-headline :filter-return #'aj/org-html-cv-trim-divs)
(advice-add 'org-html-section :filter-return #'aj/org-html-cv-trim-divs)

(defun aj/org-html-cv-trim-divs (output)
  (if aj/org-exporting-cv
      (string-trim output
                   (rx (and (* (and "<div" (* (not ">")) ">" (* space)))
                            (* (and "<p>" (* space) "</p>"))))
                   (rx (and "</div>" (* space))))
    output))
#+end_src
*** ODT-export
**** Enable citations in quote blocks, added as captions
Add a custom citation after block, taken from caption.
#+begin_src emacs-lisp
(defun aj/org-odt-quote-block (quote-block contents info)
...
We also allow for a caption, which will be put in a cite attribute."
  (format "%s%s"
          contents
          (when-let ((caption-raw (org-export-get-caption quote-block))
                     (caption (org-export-data caption-raw info)))
            (format
             "\n<text:p text:style-name=\"quote-attribution\">%s</text:p>"
             caption))))

;; Let’s just redefine the transcoder for the odt backend.
(setf (alist-get 'quote-block
                 (org-export-backend-transcoders (org-export-get-backend 'odt)))
      #'aj/org-odt-quote-block)
#+end_src
...
(defun aj/org-odt-export-noindent (data backend info)
  (when (and aj/org-export-odt-noindent
    ...
    (org-element-map data 'paragraph
      (lambda (p)
        (let* ((par-type (org-element-type (org-export-get-parent p)))
               (prev (org-export-get-previous-element p info))
               (prev-type (org-element-type prev)))
          (cond
           ((eq par-type 'item)
           ...
           ((and
             (eq par-type 'section)
             (or (not prev)
                 (and (member prev-type '(table plain-list quote-block))
                      (eq 0 (org-element-property :post-blank prev)))))
...
**** Don’t use stupid textbox for inlinetasks in odt export
Just use a paragraph style with a background and border, defined in my odt template files.
#+begin_src emacs-lisp
(setq org-odt-format-inlinetask-function #'aj/org-odt-format-inlinetask)

(defun aj/org-odt-format-inlinetask
  ...
  (concat
   ...
   (when contents
     (replace-regexp-in-string
      "text:style-name=\"[^\"]+\""
      "text:style-name=\"OrgInlineTask\""
      contents))))
#+end_src

*** LaTeX-export
**** Risky variable fix for ~org-latex-title-command~
~org-latex-title-command~ is treated as a risky variable since it ends in "-command" (see Info node: Elisp File Local Variables). This means annoying dialogs in files where it is set (and that appearently can’t be fixed by adding values to ~safe-local-variable-values~ either). This fixes it.
 #+begin_src emacs-lisp
