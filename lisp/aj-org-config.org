...
(defun aj/org-paperref-export (path _desc backend)
  (if (or (eq backend 'latex)
          (org-export-derived-backend-p backend '(latex)))
      (format (if (aj/org-paperref-find path)
                  "\\paperref{%s}"
                "\\textbf{%s}")
              path)
    path))

(defun aj/org-paperref-find (&optional ref)
  (save-excursion
    (save-match-data
      (goto-char (point-min))
...
(defun aj/org-paperref-complete (&optional _arg)
  (concat "paper:"
          (completing-read "Paperref: "
                           (save-restriction
                             (widen)
                             (aj/org-paperref-find)))))

(defun aj/org-paperref-show (path)
  (save-restriction
    (widen)
    (message (if-let ((pos (aj/org-paperref-find path)))
                 (save-excursion
                   (goto-char pos)
