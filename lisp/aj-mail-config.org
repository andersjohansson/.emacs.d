...
#+BEGIN_SRC emacs-lisp
(add-hook 'message-send-hook #'aj/confirm-send 90)

(defun aj/confirm-send ()
...
(defun aj/mml-attach-file--go-to-eob (orig-fun &rest args)
  ...
  (save-excursion
    (save-restriction
      (widen)
      (goto-char (point-max))
      (search-backward-regexp message-signature-separator (min 500 (point-max)) t) ;; 500 chars reasonable limit for signature
      (apply orig-fun args))))
...
No automatic guess if FORCE-CHOICE is non-nil."
  (let* ((user-has-chosen nil)
         (found
          (aj/mu4e-msg-account msg
            :chalmers (if-let ((tags (mu4e-message-field msg :tags)))
                          (catch 'found
                            ;; default if no throw is /chalmers/Arkiv
                            (dolist (tag tags "/chalmers/Arkiv")
