...
(use-package mu4e-alert
  ...
  (setq mu4e-alert-interesting-mail-query
        "maildir:/uu/Inbox AND flag:unread"
...
#+BEGIN_SRC emacs-lisp
(defvar aj/mu4e-get-mail-command-quick "mbsync --new -qq uu-inbox gmail-inbox")
...
(bind-key "C-S-u" #'aj/mu4e-update-mail-and-index mu4e-headers-mode-map)
#+END_SRC
...
;; sent behavior
;; (defun aj/mu4e-sent-behavior ()
;;   (if (string-match "gmail" (message-sendmail-envelope-from))
;;       'delete
;;     'sent))
;; (setq mu4e-sent-messages-behavior nil)
#+END_SRC
...
*** Signature cycling
I have long and "official" signatures defined for my accounts, but often I want to just include my name, or nothing at all. This function allows me to cycle through signature set in the context (variable ~message-context~), and a list of shorter ones.
#+BEGIN_SRC emacs-lisp
...
(setq mu4e-sent-folder "/uu/Sent" ; gmail is set to delete sent mails
      ...
      mu4e-trash-folder (lambda (msg)
                          (aj/mu4e-uu-gmail-undecided
                           msg
                           :uu "/uu/Trash"
                           :gmail "/gmail/papperskorgen"
                           :none "/uu/Trash"))
      mu4e-refile-folder #'aj/mu4e-refile-folder)

;; Utility function
(cl-defun aj/mu4e-uu-gmail-undecided (msg &key uu gmail none)
  (cond
   ((and msg
         (or
          (mu4e-message-contact-field-matches msg '(:to :from :cc :bcc) "uu.se$")
          (string-match-p (mu4e-message-field msg :maildir) "/uu/Inbox")))
    (eval uu))
   ((and msg
         (or
          (mu4e-message-contact-field-matches msg '(:to :from :cc :bcc) "mejlaandersj")
          (string-match-p (mu4e-message-field msg :maildir) "/gmail/Inbox")))
    (eval gmail))
...
(defmacro aj/mu4e~headers-defun-mark-for (mark)
  "Define a function mu4e~headers-mark-MARK, which takes a prefix argument to execute the mark immedieately."
...
(with-eval-after-load "solarized" ;; because aj/mail-tags are def such.
  (aj/mu4e-defun-toggle-tags aj/mail-tags))

(add-to-list 'mu4e-view-actions '("View in external browser" . mu4e-action-view-in-browser))
#+END_SRC

** Secure mail folder :ARCHIVE:
#+BEGIN_SRC emacs-lisp
(defun aj/browse-shf-mails ()
  (interactive)
  (ajf/mount-shf)
  (setq mu4e-maildir "~/superhemligforskning/mail/Maildir"
        mu4e-mu-home "~/superhemligforskning/mail/index/"
