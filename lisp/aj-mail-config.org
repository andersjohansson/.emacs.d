...
(defun aj/display-eudc-info-at-point ()
  (interactive)
  (if-let ((email (get-text-property (point) 'email)))
      (aj/display-eudc-info-other-buffer email)
...
(defun aj/message-pre-send-check-attachment ()
  (when (and (aj/message-says-attachment-p)
    ...
    (unless
        (y-or-n-p "Glömde du bilagan?!")
      (keyboard-quit))))

(add-hook 'message-send-hook 'aj/message-pre-send-check-attachment)
